  ...
  let%lwt n_left, n_right, n_both =
    Lwt_io.with_file ~flags:write_flags ~mode:Output path_strings (fun oc_strings ->
      Lwt_io.with_file ~flags:write_flags ~mode:Output path_json (fun oc_json ->
        ...
        in
        let%lwt n_left = write_pairs !english_only in
        let%lwt n_both = write_pairs !both in
        let%lwt n_right = String.Map.fold !other_only ~init:(Lwt.return 0) ~f:(fun ~key:line_strings ~data:() acc ->
            let%lwt () = Lwt_io.write oc_strings line_strings in
            acc >|= succ
          )
        in
        let%lwt () = Lwt_io.write oc_json "\n}\n" in
        Lwt.return (n_left, n_right, n_both)
      )
    )
  in
  Lwt_io.printlf "✅ Generated '%s' and '%s' with:\n- %d new strings\n- %d existing strings\n- %d unused strings\n"
